        ...
        <div style={{ display: "flex" }}>
          ...
          <div style={{ border: '1px solid black', width: 700, padding: 10 }}>
            {regions[regionN].description.map(
              (x, i) => (
                <div key={i} style={{ backgroundColor: i % 2 ? 'red' : "blue" }}>
                  <div>{x.title}</div>
                  <div>{x.paragraph}</div>
                </div>
              )
